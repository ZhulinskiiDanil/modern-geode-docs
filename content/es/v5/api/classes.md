---
title: 'classes'
---

## Primero la firma, después el contexto

El navegador contiene una selección pequeña verificada en v5.10.1. No es un API completo generado automáticamente. Cada símbolo enlaza a su código versionado.

## Encuentra un símbolo

Busca por nombre o namespace y filtra por tipo. Utiliza Tab y Enter. El fragmento URL identifica el símbolo para compartir enlaces directos.

## Uso seguro

Lee parámetros, tipo devuelto y ejemplo. Una vista prestada no es una cadena propia. No elimines un puntero propiedad del cargador. La disponibilidad indica una declaración en este SDK; debes compilar y probar cada plataforma.

## Continúa

Consulta el API oficial para otros overloads y clases. ApiAdapter permite sustituir la fuente de datos sin cambiar el navegador.

[API oficial](https://docs.geode-sdk.org/classes/geode/Mod/)
